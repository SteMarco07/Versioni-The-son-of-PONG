...
class Pallina:
    ...
    def disegna(self):
        print(f"pos x:{self.__pos["x"]}, pos y: {self.__pos["y"]}")
        draw_circle(int(self.__pos["x"]), int(self.__pos["y"]), self.__raggio, self.colore)

    def is_fuori_schermo(self, screen_width):
        if self.__pos["x"] < 0 or self.__pos["x"] > screen_width:
            return True
        else:
            return False

    def assegna_punto(self, screen_width):
        if self.is_fuori_schermo(screen_width):
            if self.__pos["x"] < 0:
                draw_text("Fuori, punto del G2", 500, 500, 40,BLACK)
                return 1
            else:
                draw_text("Fuori, punto del G1", 500, 500, 40,BLACK)
                return 0
        else:
            return -1