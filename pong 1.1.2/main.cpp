...
void posizione_barretta_x1(float &bx1, float vb1, float l, int screenWidth) {
...
void posizione_brarretta_x2(float &bx2, float vb2, float l, int screenWidth) {
...
void posizione_barretta_y1(float &by1, float vb1, float h, int screenHeight) {
...
void posizione_barretta_y2(float &by2, float vb2, float h, int screenHeight) {
...
void posizione_pallina_y( float &y, float raggio, float &vy, int screenHeight){
    if ( y - raggio <= 0 ) {
        vy = abs(vy);
...
void posizione_pallina_x( float &x, float raggio, float &vx, float y, float by1, float bx1, float bx2, float by2, float h, float l, bool  &rimbalzo){
...
void stato_4 (int &gioco, int &p1, int &p2, float &x, float &y, float raggio, float &v, float &vx, float &vy, float &bx1, float &bx2, float &by1, float &by2, float &vb1, float &vb2, float &h_iniz, float &h, float &l, int &tempo, int screenWidth, int screenHeight, bool &rimbalzo) {
    if (gioco == 0) {
        p1 = 0;
        p2 = 0;
        tempo = 0;
        ...
        posizione_pallina_y(y, raggio, vy, screenHeight);
        posizione_pallina_x(x, raggio, vx, y, by1, bx1, bx2, by2, h, l, rimbalzo);
        posizione_barretta_y1(by1, vb1, h, screenHeight);
        posizione_barretta_y2(by2, vb2, h, screenHeight);
...
void dis_stato_4 ( int &stato, int &gioco, int p1, int p2, float x, float y, float raggio, float bx1, float bx2, float by1, float by2, float l, float h, int screenWidth, int screenHeight) {
    ...
    for (int i = 0; i <= screenHeight; i += 100) {
        DrawRectangle((screenWidth/2)-3,i, 6, 50, WHITE);
    }
}

void dis_stato_2(int screenWidth, int screenHeight) {
    DrawText("1)    Pong classico", screenWidth*0.02, screenHeight/2, 40,WHITE);
    DrawText("2)    Pong libero", screenWidth*0.02, screenHeight/2 + 40, 40,WHITE);
}
...
int main(void)
{
    ...
    float raggio = 15, x = screenWidth / 2, y = GetRandomValue(raggio, screenHeight - raggio),v = 6, vx = 0, vy = 0;
    float h_iniz = 150, h = h_iniz, bx1 = 50, l = 15, vb1 = 7, by1 = screenHeight / 2 - h / 2;
    float bx2 = screenWidth - 50,vb2 = 7, by2 = screenHeight / 2 - h / 2;
    int p1, p2;
    ...
    while (!WindowShouldClose())
    {
        if (stato == 0) {
            ...
            stato_4(gioco, p1, p2, x, y, raggio, v, vx, vy, bx1, bx2, by1, by2, vb1, vb2, h_iniz, h, l, tempo,  screenWidth, screenHeight, rimbalzo_x);
        } else if (stato == 6) {

            if (gioco > 1 ) {
                posizione_barretta_x1(bx1, vb1, l, screenWidth);
                posizione_brarretta_x2(bx2, vb2, l, screenWidth);
            }
            stato_4(gioco, p1, p2, x, y, raggio, v, vx, vy, bx1, bx2, by1, by2, vb1, vb2, h_iniz, h, l, tempo,  screenWidth, screenHeight, rimbalzo_x);
