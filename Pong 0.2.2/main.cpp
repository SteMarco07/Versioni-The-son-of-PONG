...
void dis_colori(int screenWidth, int screenHeight){
    Color Colori[] = {BLACK, WHITE, RED, ORANGE,YELLOW,GREEN,SKYBLUE, VIOLET};
    int a = 0;
    int b;
    std::string numero;
    ...
    a=0;
    b=0;
    DrawText("1",screenWidth / 18 + 20 + a,screenHeight / 9 + 105 + b, 100, WHITE);
    for (int i = 2 ; i <= 8 ; i++ ){
        a += screenWidth/4;
        if ( i == 5){
            a=0;
...
void dis_punteggio (int p, float pos, std::vector<std::vector<Color>> colore) {
    std::string punteggio;
    punteggio = std::to_string(p);
    DrawText(punteggio.c_str(), pos, 30, 40, colore.at(2).at(0));

}

void partenza (int &gioco, int screenHeight, int screenWidth, std::vector<int> punteggio, int &stato, std::vector<std::vector<Color>> colore) {
    if (gioco == 2 && punteggio[1] != 10) {
        DrawText("Il giocatore 2 ha fatto punto", screenWidth/2.7, screenHeight/2 - 25, 40, colore.at(2).at(0));
        DrawText("Premere spazio per continuare", screenWidth/2.7, screenHeight/2 + 25, 40, colore.at(2).at(0));
    } else  if (gioco == 3 && punteggio[0] != 10) {
        DrawText("Il giocatore 1 ha fatto punto", screenWidth/2.7, screenHeight/2 - 25, 40, colore.at(2).at(0));
        DrawText("Premere spazio per continuare", screenWidth/2.7, screenHeight/2 + 25, 40, colore.at(2).at(0));
    } else if (gioco == 0) {
        DrawText("Premi spazio per iniziare", screenWidth/2.8, screenHeight/2, 40, colore.at(2).at(0));
    } else if ( punteggio[0] == 10 || punteggio[1] == 10 ) {
        if (punteggio[0] == 10) {
            DrawText("Il giocatore 1 ha vinto!", screenWidth/2.7, screenHeight/2 - 20, 40, colore.at(2).at(0));
        } else if (punteggio[1] == 10) {
            DrawText("Il giocatore 2 ha vinto!", screenWidth/2.7, screenHeight/2 - 25, 40, colore.at(2).at(0));
        }
        DrawText("Premere R per ricominciare", screenWidth/2.8, screenHeight/2 + 20, 40, colore.at(2).at(0));
        DrawText("Premere U per uscire", screenWidth/2.8, screenHeight/2 + 55, 40, colore.at(2).at(0));
        if (IsKeyDown(KEY_R)) {
            gioco = 0;
        }
        if (IsKeyDown(KEY_U)) {
...
void caricamento_dis (std::vector<float> &caricamento, int &tempo, int screenWidth, int screenHeight, int temp, int &stato, std::vector<std::vector<Color>> colore) {
    caricamento[0] += GetRandomValue(7, 25);
    if (caricamento[0] >= screenWidth) {
        stato = temp;
        caricamento[0] = 0;
    }
    DrawRectangle(0, screenHeight-caricamento[1], caricamento[0], caricamento[1], colore.at(1).at(0));
    tempo++;
    if (tempo < 15) {
        DrawText("Caricamento.", screenWidth*0.05, screenHeight-2*caricamento[1], 40, colore.at(2).at(0));
    } else if (tempo < 30) {
        DrawText("Caricamento..", screenWidth*0.05, screenHeight-2*caricamento[1], 40, colore.at(2).at(0));
    } else if (tempo < 45) {
        DrawText("Caricamento...", screenWidth*0.05, screenHeight-2*caricamento[1], 40, colore.at(2).at(0));
...
void stato_0 (int screenWidth, int screenHeight, std::vector<std::vector<Color>> colore){
    DrawText("PONG", screenWidth*0.45, screenHeight/3, 100, colore.at(2).at(0));
    DrawText("Premi M per scegliere la modalita' della partita", screenWidth*0.02, screenHeight*0.70, 50, colore.at(2).at(0));
    DrawText("Premi I per vedere le impostazioni", screenWidth*0.02, screenHeight*0.80, 50, colore.at(2).at(0));
}
...
void dis_stato_4 ( int &stato, int &gioco, std::vector<int> punteggio, std::vector<float> pallina, float raggio, std::vector<float> barretta, float l, float h,
                   int screenWidth, int screenHeight, std::vector<std::vector<Color>> colore) {
    partenza(gioco, screenHeight, screenWidth, punteggio, stato, colore);

    //Color Colori[] = {WHITE, RED, ORANGE, YELLOW, GREEN, SKYBLUE, PINK, VIOLET,};

    DrawCircle(pallina[0], pallina[1], raggio, colore.at(0).at(0));
    DrawRectangle(barretta[0], barretta[1] , l, h, colore.at(1).at(0));
    DrawRectangle(barretta[2] - l, barretta[3] , l, h, colore.at(1).at(0));
    dis_punteggio(punteggio[0], screenWidth / 4, colore);
    dis_punteggio(punteggio[1], screenWidth  * 0.75, colore);
    for (int i = 0; i < screenHeight; i += 60) {
        DrawRectangle((screenWidth/2)-3,i, 6, 20, colore.at(1).at(0));
    }
}

void dis_stato_2(int screenWidth, int screenHeight, std::vector<std::vector<Color>> colore) {
    Color prova = colore.at(2).at(0);
    DrawText("Modalita'", screenWidth*0.40, screenHeight*0.02, 100,prova);
    DrawText("1)     Pong classico", screenWidth*0.02, screenHeight/2 - 200, 60,prova);
    DrawText("2)    Pong libero", screenWidth*0.02, screenHeight/2 - 120, 60,prova);
    DrawText("3)    Pong classico medio", screenWidth*0.02, screenHeight/2 - 40, 60,prova);
    DrawText("4)    Pong libero medio", screenWidth*0.02, screenHeight/2 + 40, 60,prova);
    DrawText("5)    Pong classico difficile", screenWidth*0.02, screenHeight/2 + 120, 60,prova);
    DrawText("6)    Pong libero difficile", screenWidth*0.02, screenHeight/2 + 200, 60,prova);
}
...
void dis_stato_6 (int &stato,int &gioco, std::vector<int> punteggio, std::vector<float> pallina, float raggio, std::vector<float> barretta, float l, float h,
                  float screenWidth, float screenHeight, std::vector<std::vector<Color>> colore) {
    dis_stato_4(stato, gioco, punteggio, pallina, raggio, barretta, l, h, screenWidth, screenHeight, colore);
    for (int i = 0; i < screenHeight; i += 80) {
        DrawRectangle((screenWidth/3)-3,i, 6, 20, colore.at(1).at(0));
    }
    for (int i = 0; i < screenHeight; i += 80) {
        DrawRectangle((screenWidth*2/3)-3,i, 6, 20, colore.at(1).at(0));
    }
}
...
void scelta_elemento_colorare(int &stato ){
    if (IsKeyDown(KEY_A)) {
        stato = 22;
...
void selezione_colore (int &stato) {
    if (IsKeyDown(KEY_ONE)) {
        stato = 21;
...
int main(void)
{
    ...
    std::vector<std::vector<Color>> colore {{WHITE, WHITE},
                               {WHITE, BLACK},                                                          //pallina, barrette + linee, scritte + punti, sfondo
                               {WHITE, WHITE},
                               {BLACK, BLACK}};
    ...
    while (!WindowShouldClose()) {
        ...
        if (stato == 0) {
            stato_0(screenWidth, screenHeight, colore);
        } else if (stato == 1) {
            caricamento_dis(caricamento, tempo, screenWidth, screenHeight, 2, stato, colore);
        } else if (stato == 2) {
            dis_stato_2(screenWidth, screenHeight, colore);
        } else if (stato == 3) {

            caricamento_dis(caricamento, tempo, screenWidth, screenHeight, 4, stato, colore);
            ...
            caricamento_dis(caricamento, tempo, screenWidth, screenHeight, 6, stato, colore);
            difficolta(0, velocita, h_iniz,raggio);

        } else if (stato == 6) {

            dis_stato_6(stato, gioco, punteggio, pallina, raggio, barretta, l, h, screenWidth, screenHeight, colore);

        } else if (stato == 7) {

            caricamento_dis(caricamento, tempo, screenWidth, screenHeight, 8, stato, colore);
            ...
            caricamento_dis(caricamento, tempo, screenWidth, screenHeight, 10, stato, colore);
            difficolta(1, velocita, h_iniz,raggio);
            ...
            caricamento_dis(caricamento, tempo, screenWidth, screenHeight, 12, stato, colore);
            ...
            caricamento_dis(caricamento, tempo, screenWidth, screenHeight, 14, stato, colore);
            difficolta(2, velocita, h_iniz,raggio);
            ...
            caricamento_dis(caricamento, tempo, screenWidth, screenHeight, 21, stato, colore);
        } else if (stato == 21) {
            dis_colori0(screenWidth, screenHeight);
        } else if (stato == 22) {
            dis_colori(screenWidth, screenHeight);
        }
        if (stato == 25)
            caricamento_dis(caricamento, tempo, screenWidth, screenHeight, 0, stato, colore);
        ClearBackground(colore.at(3).at(0));
