...
void dis_scelta_elemento_colorare_stato_21 (const Game &game, const String &stringhe, const Colors &colori){
    float screenHeight = game.screenHeight/2 - 180;
    ...
    for (int i = 1; i <= 4; ++i) {
        DrawText(stringhe.frasi.at(21).at(i).c_str(), game.screenWidth * 0.02, screenHeight, stringhe.dim_stringhe.at(2),colore);
        screenHeight += 100;
    }
}
...
void dis_home_stato_0 (const Game &game, const Colors &colori, const String &stringhe){
    int screenWidth = game.screenWidth, screenHeight = game.screenHeight;
    Color colore = colori.colore_elementi.at(colori.scelta_elemento.at(2));
    DrawText(stringhe.frasi.at(0).at(0).c_str(), screenWidth * 0.40, screenHeight * 0.02, stringhe.dim_stringhe.at(0), colore);
...
void dis_modalita_stato_2(const Game &game, const Colors &colori, const String &stringhe) {
    ...
    for (int i = 1; i < 8; ++i) {
        DrawText(stringhe.frasi.at(2).at(i).c_str(), screenWidth * 0.02, screenHeight, stringhe.dim_stringhe.at(2), colore);
        screenHeight+= 100;
    }
...
void modalita_stato_2 (Ball &pallina, Speeds &velocita, Game &game , const String &stringhe) {
    float dimensione = stringhe.dim_stringhe.at(2), screenHeight = game.screenHeight/2-280, screenWidth = game.screenWidth*0.02;
    for (int i = 1; i < 8; ++i) {
        if (CheckCollisionPointRec(GetMousePosition(), {screenWidth, screenHeight, (float)MeasureText(stringhe.frasi.at(2).at(i).c_str(), dimensione), dimensione}) &&
            IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
            if (i < 3) {
...
void scelta_elemento_colorare_stato_21(const String &stringhe, Game &game, Colors &colori){
    float x = game.screenWidth*0.02 + 190;
    float y = game.screenHeight/2;
...
void impostazioni_stato_20 (Game &game, const String &stringhe) {
    float x = game.screenWidth * 0.02, y = game.screenHeight;
