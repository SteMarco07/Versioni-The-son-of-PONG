...
void dis_colori( const Game &game, const String &stringhe, const Colors &colori){
    ...
    for (int i = 0; i <= colori.colore_elementi.size(); ++i) {
        if (i >= 4)
        ...
        if (i < colori.colore_elementi.size()) {
            ...
            DrawRectangle(x / 18 + 5 + a, y / 9 + 105 + b, x / 7.2-10, y / 3.6-10, colori.colore_temp.at(game.elemento_da_colorare));
        }
        a += x / 4;
    }
    ...
    for (int i = 0; i <= colori.colori_partenza.size(); ++i) {
        ...
        if (i < colori.colori_partenza.size()) {
            if (CheckCollisionPointRec(GetMousePosition(), {x, y, lato, lato}) && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)){
                game.stato = 21;
                colori.scelta_elemento.at(game.elemento_da_colorare) = game.elemento_da_colorare;
                colori.colore_elementi.at(game.elemento_da_colorare) = colori.colori_partenza.at(i);
            }
        } else{
            if (CheckCollisionPointRec(GetMousePosition(), {x, y, lato, lato}) && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)){
                game.stato = 23;
...
void interruzioni (Game &game, const Players &giocatori, const Colors &colori, const String &stringhe) {
    ...
    } else if ( giocatori.giocatore_1.punteggio == 10 || giocatori.giocatore_2.punteggio == 10 ) {
...
void stato_3 (Game &game, Players &giocatori, Ball &pallina, Speeds &velocita) {
    float h = giocatori.giocatore_1.barretta.height;
    int x = GetScreenWidth(), y = GetScreenHeight();
    if (game.gioco == 0) {
        giocatori.giocatore_1.punteggio = 9;
        ...
        if (IsKeyPressed(KEY_SPACE) && game.gioco == 3) {
            vi_y(velocita);
            velocita.v_ball_x = velocita.v0;
            repet(game, giocatori);
            pallina.rimbalzo = false;
        } else if (IsKeyPressed(KEY_SPACE) && game.gioco == 2) {
            vi_y(velocita);
            velocita.v_ball_x = -velocita.v0;
            repet(game, giocatori);
            pallina.rimbalzo = true;
...
void dis_stato_3 (Game &game, const Ball &pallina,  Players &giocatori, const Colors &colori, const String &stringhe) {
    ...
    if (!(game.gioco == 0 || game.gioco == 2 || game.gioco == 3)) {
...
void stato_4 (Game &game,Players &giocatori,Ball &pallina,Speeds &velocita) {
    stato_3(game, giocatori, pallina, velocita);
...
void colori_custom (float x, float pos_partenza, const Game &game, Colors &colori) {
    if (IsMouseButtonDown(MOUSE_BUTTON_LEFT) && CheckCollisionPointRec(GetMousePosition(), {x , pos_partenza, 40, 255})){
        colori.canali_colori.at(game.elemento_da_colorare*3) = GetMouseY() - pos_partenza;
    }
    if (IsMouseButtonDown(MOUSE_BUTTON_LEFT) && CheckCollisionPointRec(GetMousePosition(), {x + 80 , pos_partenza, 40, 255})){
        colori.canali_colori.at(game.elemento_da_colorare*3 + 1) = GetMouseY() - pos_partenza;
    }
    if (IsMouseButtonDown(MOUSE_BUTTON_LEFT) && CheckCollisionPointRec(GetMousePosition(), {x + 160 , pos_partenza, 40, 255})){
        colori.canali_colori.at(game.elemento_da_colorare*3 + 2) = GetMouseY() - pos_partenza;
    }

    colori.colore_temp.at(game.elemento_da_colorare) = {colori.canali_colori.at(game.elemento_da_colorare*3), colori.canali_colori.at(game.elemento_da_colorare*3 + 1), colori.canali_colori.at(game.elemento_da_colorare*3 + 2), 255};
}
...
void dis_colori_custom (const Colors &colori, const String &stringhe, const Game &game, float x, float pos_partenza) {
    ...
    DrawRectangle(x + 5, colori.canali_colori.at(game.elemento_da_colorare*3) + pos_partenza, 30, 10, BLACK);
    DrawRectangle(x + 80, pos_partenza, 40, 256 + 10, GREEN);
    DrawRectangle(x + 85, colori.canali_colori.at(game.elemento_da_colorare*3+1) + pos_partenza, 30, 10, BLACK);
    DrawRectangle(x + 160, pos_partenza, 40, 256 + 10, BLUE);
    DrawRectangle(x + 165, colori.canali_colori.at(game.elemento_da_colorare*3+2) + pos_partenza, 30, 10, BLACK);

    DrawRectangle(x - 10, pos_partenza + 306, 220, 50, GRAY);
    std::string rosso;
    rosso = std::to_string(colori.canali_colori.at(game.elemento_da_colorare*3));
    DrawText(rosso.c_str(), x - 5, pos_partenza + 316, 30, RED);
    std::string verde;
    verde = std::to_string(colori.canali_colori.at(game.elemento_da_colorare*3+1));
    DrawText(verde.c_str(), x + 75, pos_partenza + 316, 30, GREEN);
    std::string blu;
    blu = std::to_string(colori.canali_colori.at(game.elemento_da_colorare*3+2));
    DrawText(blu.c_str(), x + 155, pos_partenza + 316, 30, BLUE);

    DrawRectangle(lunghezza*4/5 - 266, pos_partenza-10, 276, 276, GRAY);
    DrawRectangle(lunghezza*4/5 - 256, pos_partenza, 256, 256, colori.colore_temp.at(game.elemento_da_colorare));
...
void dis_stato_26 (const String &stringhe, const Colors &colori) {
    ...
    DrawText(stringhe.frasi.at(26).at(1).c_str(),  x/2 - MeasureText(stringhe.frasi.at(26).at(1).c_str(), stringhe.dim_stringhe.at(2))/2, (y - stringhe.dim_stringhe.at(2))/2, stringhe.dim_stringhe.at(2), colori.colore_elementi.at(colori.scelta_elemento.at(2)));
}
...
int main(void)
{
    ...
    Game game = {0, 0, 0, h_iniz, 0, 0,0, 0, 0, 0};
    ...
    String stringhe {
            {
                    ...
                    {""},
                    {""},
                    {"Impostazioni", "1)     Modifica i nomi","2)    Colori", "3)    Crediti"},
    ...
    while (!WindowShouldClose()) {
        ...
        if (game.stato == 0) {
            ...
            scelta_difficolta(pallina, velocita, game);
            stato_4(game, giocatori, pallina, velocita);
            ...
            if (colori.colorare) {
                colori.colore_elementi.at(game.elemento_da_colorare) = colori.colore_temp.at(game.elemento_da_colorare);
