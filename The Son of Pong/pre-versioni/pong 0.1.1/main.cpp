...
void pbx2(float &bx2, float vb2, float l, int screenWidth) {
    if (IsKeyDown(KEY_LEFT) && bx2 > screenWidth*2/3) {
        bx2 -= vb2;
    } else if (IsKeyDown(KEY_RIGHT) && bx2 + l < screenWidth) {
        bx2 += vb2;
    }
}
...
void stato_4 (int &gioco, int &p1, int &p2, float &x, float &y, float raggio, float &v, float &vx, float &vy, float &bx1, float &bx2, float &by1, float &by2, float &vb1, float &vb2, float &h_iniz, float &h, float &l, int &tempo, int screenWidth, int screenHeight) {
    if (gioco == 0) {
        p1 = 0;
        p2 = 0;
        tempo = 0;
...
int main(void)
{
    ...
    while (!WindowShouldClose())
    {
        ...
        if (stato == 0) {
            ...
            stato_4(gioco, p1, p2, x, y, raggio, v, vx, vy, bx1, bx2, by1, by2, vb1, vb2, h_iniz, h, l, tempo,  screenWidth, screenHeight);
        } else if (stato == 6) {
            stato_4(gioco, p1, p2, x, y, raggio, v, vx, vy, bx1, bx2, by1, by2, vb1, vb2, h_iniz, h, l, tempo,  screenWidth, screenHeight);
            if (gioco > 1 ) {
