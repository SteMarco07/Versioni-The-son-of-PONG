...
void pbx1(const Speeds &velocita, Players &giocatori) {
    ...
    if (IsKeyDown(KEY_A) && giocatori.giocatore_1.barretta.x - l/2> 0) {
...
void pbx2(const Speeds &velocita, Players &giocatori) {
    int x = GetScreenWidth();
    float l = giocatori.giocatore_2.barretta.width;
    if (IsKeyDown(KEY_LEFT) && giocatori.giocatore_2.barretta.x > x*2/3) {
        giocatori.giocatore_2.barretta.x -= velocita.v_barretta_2;
    } else if (IsKeyDown(KEY_RIGHT) && giocatori.giocatore_2.barretta.x + l < x) {
        giocatori.giocatore_2.barretta.x += velocita.v_barretta_2;
    }
}
...
void repet (Game &game, Players &giocatori) {
    ...
    giocatori.giocatore_2.barretta.height = game.h_iniz;
}
...
void stato_3 (Game &game, Players &giocatori, Ball &pallina, Speeds &velocita, const String &stringhe) {
    ...
    if (!game.pausa) {
        if (game.gioco == 0) {
            ...
            game.tempo_partita = 0;
            giocatori.giocatore_1.barretta = {50, y / 2 - game.h_iniz / 2, giocatori.giocatore_1.barretta.width, game.h_iniz};
            giocatori.giocatore_2.barretta = {(float)(x - 50), y / 2 - game.h_iniz / 2, giocatori.giocatore_1.barretta.width, game.h_iniz};
            if (pallina.x != x/2) {
                pallina.x = x/2;
            ...
            if (game.tempo_partita > 1500 && game.gioco != 2 && game.gioco != 3) {
                ...
                if (game.tempo_partita > 2000 && giocatori.giocatore_1.barretta.height > 30) {
                    giocatori.giocatore_1.barretta.height -= 0.05;;
                    giocatori.giocatore_2.barretta.height -= 0.05;;
...
void dis_stato_2(const Colors &colori, const String &stringhe) {
    ...
    DrawText(stringhe.frasi.at(2).at(7).c_str(), x*0.02, y/2 + 320, stringhe.dim_stringhe.at(2),colore);
...
void stato_2 (float x, float y, Game &game , const String &stringhe) {
    ...
    if (CheckCollisionPointRec(GetMousePosition(), {x, y - 280, (float)MeasureText(stringhe.frasi.at(2).at(1).c_str(), dimensione), dimensione}) &&
        ...
        game.stato_successivo = 4;
        game.difficolta = 2;
    } else if (CheckCollisionPointRec(GetMousePosition(), {static_cast<float>(GetScreenWidth()*0.02), y + 320, (float)MeasureText(stringhe.frasi.at(2).at(7).c_str(), dimensione), dimensione}) &&
              IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
        game.stato= 19;
    }
}
...
void colori_custom (float x, float pos_partenza, const Game &game, Colors &colori) {
    ...
    colori.colori_custom.at(colori.elemento_da_colorare) = {colori.canali_colori.at(colori.elemento_da_colorare*3), colori.canali_colori.at(colori.elemento_da_colorare*3 + 1), colori.canali_colori.at(colori.elemento_da_colorare*3 + 2), 255};
}
...
void riempi_colori_custom (Colors &colori){
    for (int i = 0; i < colori.canali_colori.size()/3; ++i) {
        colori.colori_custom.at(i) = {colori.canali_colori.at(i*3), colori.canali_colori.at(i*3 + 1), colori.canali_colori.at(i * 3 + 2), 255};
    }
...
void dis_stato_27 (const Game &game, const String &stringhe, const Colors &colori) {
    float x = GetScreenWidth(), y = GetScreenHeight();
    Color colore = colori.colore_elementi.at(colori.scelta_elemento.at(2));
...
void dis_stato_19 (const String &stringhe, const Colors &colori) {
    float x = GetScreenWidth(), y = GetScreenHeight();
...
int main(void)
{
    ...
    Game game = {0, 0, 0, h_iniz, 0, 0,0, 0, 0, false, 0};
    ...
    while (!WindowShouldClose()) {
        ...
        if (game.stato == 0) {
            ...
            dis_stato_27 (game, stringhe, colori);
            dis_sposta_tra_stati(triangolo_indietro, colori);
            if (game.pagina_tasti < 2) {
